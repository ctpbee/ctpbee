...
from vnpy.api.ctp import TdApi, defineDict
...
from vtEvent import *
from .constant import VtOrderData
...
class CtpTdApi(TdApi):
    ...
    def onRspAuthenticate(self, data, error, n, last):
        ...
        if error['ErrorID'] == 0:
            ...
            event = Event(err, type_=EVENT_ERROR)
    ...
    def onRspOrderInsert(self, data, error, n, last):
        ...
        order_event = Event(order, EVENT_ORDER)
    ...
    def onRspQryOrder(self, data, error, n, last):
        ...
        if not data['InstrumentID']:
            return
        order_name = self.gatewayName + data['OrderRef']
        if order_name in self.orderDict:
            order = self.orderDict[order_name]
        else:
            order = VtOrderData()
            self.orderDict[order_name] = order
            order.gatewayName = self.gatewayName
            order.symbol = data['InstrumentID']
            order.exchange = exchangeMapReverse[data['ExchangeID']]
            order.price = data['LimitPrice']
            ...
            order.vtSymbol = data['InstrumentID']
        ...
        ref = int(data["OrderRef"])
        if ref not in self.orderRefList:
            self.orderRefList.append(ref)

        if last:
            for pos in self.orderDict.values():
                event = Event(pos, EVENT_ORDER)
                self.event_engine.put(event)
    ...
    def onRspQryTrade(self, data, error, n, last):
        ...
        if not data['InstrumentID']:
            return
        trade_name = str(data['BrokerOrderSeq']) + str(data['TradeID'])
        if trade_name in self.tradeDict:
            trade = self.tradeDict[trade_name]
        else:
            trade = VtTradeData()
            self.tradeDict[trade_name] = trade
            ...
            trade.direction = directionMapReverse.get(data['Direction'], OFFSET_UNKNOWN)
            trade.offset = offsetMapReverse.get(data['OffsetFlag'], OFFSET_UNKNOWN)
        ...
        if last:
            for pos in self.tradeDict.values():
                event = Event(pos, EVENT_TRADE)
                self.event_engine.put(event)
    ...
    def onRspQryInvestorPosition(self, data, error, n, last):
        ...
        if last:
            ...
            for pos in self.posDict.values():
                event = Event(pos, EVENT_POSITION)
                self.event_engine.put(event)
    ...
    def onRspQryTradingAccount(self, data, error, n, last):
        ...
        event = Event(account, EVENT_ACCOUNT)
        self.event_engine.put(event)
    ...
    def onRspQryInstrument(self, data, error, n, last):
        ...
        # 推送
        event = Event(contract, EVENT_CONTRACT)
        self.event_engine.put(event)
    ...
    def onErrRtnOrderInsert(self, data, error):
        ...
        order.price = data['LimitPrice']
        order.totalVolume = data['VolumeTotalOriginal']
        event_order = Event(order, EVENT_ORDER)
    ...
    def sendOrder(self, orderReq):
        ...
        req['VolumeTotalOriginal'] = int(orderReq.volume)  # 手数
