...
class MACDStrategy(CtpbeeApi):
    ...
    def on_bar(self, bar: BarData):
        ...
        # 计算MACD
        macd_line, signal_line, histo = macd(self.close, self.fast_period, self.slow_period, self.signal_period)
        ...
        self.logger.debug(f"MACD计算 - MACD线: 当前 {current_macd:.2f}, 前值 {previous_macd:.2f}; 信号线: 当前 {current_signal:.2f}, 前值 {previous_signal:.2f}; 柱状图: 当前 {current_histo:.2f}", owner=self.name)
        ...
        if buy_signal:
            self.logger.info(f"买入信号 - MACD金叉形成: MACD线 {current_macd:.2f} 上穿信号线 {current_signal:.2f}, 柱状图 {current_histo:.2f}", owner=self.name)
        if sell_signal:
            self.logger.info(f"卖出信号 - MACD死叉形成: MACD线 {current_macd:.2f} 下穿信号线 {current_signal:.2f}, 柱状图 {current_histo:.2f}", owner=self.name)
