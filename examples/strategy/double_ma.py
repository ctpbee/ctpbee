...
class DoubleMa(CtpbeeApi):
    ...
    def __init__(self, name: str, code):
        ...
        self.name = name
        self.logger.info(
            f"双均线策略初始化 - 合约: {code}, 快速周期: {self.fast_period}, 慢速周期: {self.slow_period}",
            owner=self.name,
        )
    ...
    def on_bar(self, bar: BarData):
        ...
        self.logger.debug(
            f"MA计算 - 快速MA: 当前 {current_fast:.2f}, 前值 {previous_fast:.2f}; 慢速MA: 当前 {current_slow:.2f}, 前值 {previous_slow:.2f}",
            owner=self.name,
        )
        ...
        if buy:
            self.logger.info(
                f"买入信号 - 金叉形成: 快速MA {current_fast:.2f} 上穿慢速MA {current_slow:.2f}",
                owner=self.name,
            )
        if sell:
            self.logger.info(
                f"卖出信号 - 死叉形成: 快速MA {current_fast:.2f} 下穿慢速MA {current_slow:.2f}",
                owner=self.name,
            )
