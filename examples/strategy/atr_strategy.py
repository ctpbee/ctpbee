...
class ATRStrategy(CtpbeeApi):
    ...
    def on_bar(self, bar: BarData):
        ...
        self.logger.debug(f"指标计算 - SMA: {current_sma:.2f}, ATR: {current_atr:.2f}, 最近10日最大ATR: {max_recent_atr:.2f}, 趋势: {'上涨' if trend_up else '下跌' if trend_down else '横盘'}", owner=self.name)
        
        # 交易逻辑
        if self.pos == 0:
            # 买入信号：上涨趋势且ATR较小（波动性低）
            if trend_up and current_atr < max_recent_atr:
                self.logger.info(f"买入信号 - 上涨趋势且波动性降低: SMA {current_sma:.2f}, 价格 {bar.close_price:.2f}, ATR {current_atr:.2f} < 最近10日最大ATR {max_recent_atr:.2f}", owner=self.name)
                self.pos = 1
                self.entry_price = bar.close_price
                self.stop_loss = self.entry_price - current_atr * self.stop_loss_multiplier
                self.take_profit = self.entry_price + current_atr * self.take_profit_multiplier
                self.logger.success(f"开仓买入 - 合约: {bar.local_symbol}, 价格: {bar.close_price}, 止损: {self.stop_loss:.2f}, 止盈: {self.take_profit:.2f}, 当前仓位: {self.pos}", owner=self.name)
                self.action.buy_open(bar.close_price, 1, bar)
            # 卖出信号：下跌趋势且ATR较小（波动性低）
            elif trend_down and current_atr < max_recent_atr:
                self.logger.info(f"卖出信号 - 下跌趋势且波动性降低: SMA {current_sma:.2f}, 价格 {bar.close_price:.2f}, ATR {current_atr:.2f} < 最近10日最大ATR {max_recent_atr:.2f}", owner=self.name)
                self.pos = -1
                self.entry_price = bar.close_price
                self.stop_loss = self.entry_price + current_atr * self.stop_loss_multiplier
                self.take_profit = self.entry_price - current_atr * self.take_profit_multiplier
                self.logger.success(f"开仓卖出 - 合约: {bar.local_symbol}, 价格: {bar.close_price}, 止损: {self.stop_loss:.2f}, 止盈: {self.take_profit:.2f}, 当前仓位: {self.pos}", owner=self.name)
                self.action.sell_open(bar.close_price, 1, bar)
        elif self.pos == 1:
            # 多头止损和止盈
            if bar.low_price <= self.stop_loss:
                self.logger.info(f"多头止损触发 - 合约: {bar.local_symbol}, 当前最低价: {bar.low_price:.2f} <= 止损价: {self.stop_loss:.2f}", owner=self.name)
                ...
                self.logger.info(f"多头止盈触发 - 合约: {bar.local_symbol}, 当前最高价: {bar.high_price:.2f} >= 止盈价: {self.take_profit:.2f}", owner=self.name)
                self.logger.success(f"平仓卖出 - 合约: {bar.local_symbol}, 价格: {bar.close_price}, 当前仓位: {self.pos} -> 0", owner=self.name)
                self.action.buy_close(bar.close_price, 1, bar)
                self.pos = 0
        elif self.pos == -1:
            ...
            if bar.high_price >= self.stop_loss:
                self.logger.info(f"空头止损触发 - 合约: {bar.local_symbol}, 当前最高价: {bar.high_price:.2f} >= 止损价: {self.stop_loss:.2f}", owner=self.name)
                ...
                self.logger.info(f"空头止盈触发 - 合约: {bar.local_symbol}, 当前最低价: {bar.low_price:.2f} <= 止盈价: {self.take_profit:.2f}", owner=self.name)
                self.logger.success(f"平仓买入 - 合约: {bar.local_symbol}, 价格: {bar.close_price}, 当前仓位: {self.pos} -> 0", owner=self.name)
                self.action.sell_close(bar.close_price, 1, bar)
                self.pos = 0
