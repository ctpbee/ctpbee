...
class RSIStrategy(CtpbeeApi):
    ...
    def on_bar(self, bar: BarData):
        ...
        self.logger.debug(f"RSI计算 - 当前值: {current_rsi:.2f}, 前值: {previous_rsi:.2f}, 超买: {self.over_bought}, 超卖: {self.over_sold}", owner=self.name)
        
        # 超卖买入，超买卖出
        buy_signal = current_rsi > self.over_sold and previous_rsi <= self.over_sold
        sell_signal = current_rsi < self.over_bought and previous_rsi >= self.over_bought
        
        if buy_signal:
            self.logger.info(f"买入信号 - RSI从 {previous_rsi:.2f} 上穿超卖阈值 {self.over_sold}，当前值 {current_rsi:.2f}", owner=self.name)
        if sell_signal:
            self.logger.info(f"卖出信号 - RSI从 {previous_rsi:.2f} 下穿超买阈值 {self.over_bought}，当前值 {current_rsi:.2f}", owner=self.name)
