...
def cci(close, high, low, period=20):
    ...
    for i in range(period, n):
        # 计算典型价格
        typical_price = (high[i-period:i] + low[i-period:i] + close[i-period:i]) / 3
...
def rvi(close, high, low, period=10):
    ...
    # 计算分子和分母
    numerator = (diff_close[-period:] + 2*diff_close[-period-1:-1] + 2*diff_close[-period-2:-2] + diff_close[-period-3:-3]) / 6
    denominator = (diff_high[-period:] + 2*diff_high[-period-1:-1] + 2*diff_high[-period-2:-2] + diff_high[-period-3:-3]) / 6
    denominator = denominator - (diff_low[-period:] + 2*diff_low[-period-1:-1] + 2*diff_low[-period-2:-2] + diff_low[-period-3:-3]) / 6
