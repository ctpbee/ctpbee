# coding: utf-8
# from DataRecording
from __future__ import print_function
import multiprocessing
from time import sleep
from datetime import datetime, time
from vnpy.trader.vtEngine import LogEngine
from App import App
from App.tick.mdApi import CtpMdApi
from App.tick.engine import EventEngine2


def runApp(le):
    event_engine = EventEngine2()
    app = App(name=__name__, event_engine=event_engine, mdApi=CtpMdApi, le=le)
    # 载入配置文件
    app.load_settings_from_py_file("config.py")

    # 选择数据库连接
    le.info("初始化 BAR 数据库连接")
    app.init_bar("mongo")
    le.info("初始化 TICK 数据库连接")
    app.init_tick("mongo")

    # 连接ctp
    le.info("连接ctp")
    app.connect_ctp()
    # 开始定时器引擎
    app.start()
    while True:
        sleep(1)


def runChildProcess():
    """子进程运行函数"""
    # 创建日志引擎
    le = LogEngine()
    le.setLogLevel(le.LEVEL_INFO)
    le.addConsoleHandler()
    le.info(u'启动行情记录运行子进程')
    runApp(le)


def runParentProcess():
    """父进程运行函数"""
    # 创建日志引擎
    le = LogEngine()
    le.setLogLevel(le.LEVEL_INFO)
    le.addConsoleHandler()
    le.info(u'Hello Ding')
    DAY_START = time(8, 57)  # 日盘启动和停止时间
    DAY_END = time(15, 18)
    NIGHT_START = time(20, 57)  # 夜盘启动和停止时间
    NIGHT_END = time(2, 33)

    p = None  # 子进程句柄

    while True:
        currentTime = datetime.now().time()
        recording = False

        # 判断当前处于的时间段
        if ((currentTime >= DAY_START and currentTime <= DAY_END) or
                (currentTime >= NIGHT_START) or
                (currentTime <= NIGHT_END)):
            recording = True

        # 过滤周末时间段：周六全天，周五夜盘，周日日盘
        if ((datetime.today().weekday() == 6) or
                (datetime.today().weekday() == 5 and currentTime > NIGHT_END) or
                (datetime.today().weekday() == 0 and currentTime < DAY_START)):
            recording = False

        # 记录时间则需要启动子进程
        if recording and p is None:
            le.info(u'启动子进程')
            p = multiprocessing.Process(target=runChildProcess)
            p.start()
            le.info(u'子进程启动成功')
        # 非记录时间则退出子进程
        if not recording and p is not None:
            le.info(u'关闭子进程')
            p.terminate()
            p.join()
            p = None
            le.info(u'子进程关闭成功')
        sleep(5)


if __name__ == '__main__':
    runParentProcess()
