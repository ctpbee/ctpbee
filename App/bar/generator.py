...
class BarGenerator(object):
    ...
    def updateTick(self, tick):
        """TICK更新"""
        newBar = False  # 默认不是新的5秒
        # 尚未创建对象
        if not self.bar:
            self.bar = VtBarData()
            newBar = True
        # 新的5秒钟
        elif abs(tick.datetime.second - self.bar.datetime.second) >= 5: # 5秒钟走完
    ...
    def updateBar(self, bar):
        ...
        # 1分钟已经走完
        if bar.datetime.minute - self.xMinBar.datetime.minute >= 1:  # 相隔一分钟
            # 生成上一X分钟K线的时间戳
            self.xMinBar.datetime = self.xMinBar.datetime.replace(microsecond=0)  # 将秒和微秒设为0
